...
import type { VoiceParseResponse, VoiceExecuteResponse } from '@/lib/types'
import { toast } from 'sonner'

type RecordingState = 'idle' | 'listening' | 'processing' | 'review' | 'error'

interface ParsedAction {
  type: 'task' | 'event' | 'reminder' | 'intent'
  title: string
  date?: string
  time?: string
  category?: string
...
export function RecorderPanel({ className }: RecorderPanelProps) {
  ...
  const [rawParsedAction, setRawParsedAction] = useState<VoiceParseResponse['parsedAction'] | null>(null)
  ...
  const parseMutation = useMutation({
    mutationFn: (transcript: string) => {
      ...
      return voiceAPI.parse(transcript, new Date().toISOString(), timezone)
    },
    onError: (error: any) => {
      toast.error(error.message || 'Failed to parse transcript')
  ...
  const executeMutation = useMutation({
    mutationFn: voiceAPI.execute,
    onSuccess: (response: VoiceExecuteResponse) => {
      toast.success('Action executed successfully!')
      queryClient.invalidateQueries({ queryKey: ['tasks'] })
      queryClient.invalidateQueries({ queryKey: ['calendar'] })
      queryClient.invalidateQueries({ queryKey: ['life-organization'] })
      setState('idle')
      setTranscript('')
      setParsedAction(null)
      setRawParsedAction(null)
      setDuration(0)
    },
    onError: (error: any) => {
  ...
  const handleConfirm = useCallback(() => {
    if (!rawParsedAction) return
    
    // Execute the action
    executeMutation.mutate(rawParsedAction)
  }, [rawParsedAction, executeMutation])
